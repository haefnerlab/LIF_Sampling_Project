...
if __name__ == "__main__":
    parser = OptionParser(usage="Usage: %prog [options] <Gratings.npy> <trans_mat.npy> contrast")
    ...
    # Open input file
    in_fname = args[0]
    in_patches = np.load(in_fname)

    trans_fname = args[1]
    trans_mat = np.load(trans_fname)
    # in_h5 = tables.open_file(in_fname, "r")

    # in_patches = in_h5.root.patches
    # in_oversized = in_h5.root.patches

    #in_patches = np.load('Gratings_latest5_2.5.npy')
    in_patches = np.reshape(in_patches,(21*21,8,8))
    ...
    # Output file name
    out_fname = "grating"+"-patches-%d" % size
    # if options.mf:
    out_fname += "-mf"
    # if options.norm:
    #     out_fname += "-norm"
    ...
    np.save('GratingIm-mf-'+'sf'+in_fname[15]+'.npy',P)
