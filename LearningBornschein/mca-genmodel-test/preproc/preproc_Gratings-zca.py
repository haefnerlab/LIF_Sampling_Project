...
if __name__ == "__main__":
    parser = OptionParser(usage="Usage: %prog [options] <Gratings.npy>")
    ...
    sz_all = np.prod(np.shape(in_patches)[:len(np.shape(in_patches))-1])
    in_patches = np.reshape(in_patches,(sz_all,8,8))
