...
from pulp.analyze.gabor_fitting import gabor_kern
from pulp.analyze.dog_fitting import dog_kern, find_globulars
...
if __name__ == "__main__":
    ...
    h5 = tables.open_file(outdir+"/result.h5", "r")
    ...
    if options.pdf:
        ...
        pylab.cla()
        H2 = int(np.sqrt(H))+1
        # fig, axes = pylab.subplots(nrows=H2, ncols=H2)
        fig, axes = pylab.subplots(nrows=10, ncols=50)
        h=0
        for ax in axes.flat:
            ax.set_axis_off()
            if(h<xrange(H)):
                ...
                # pylab.axis("off");
                im = ax.imshow(W[-1,h], vmin=-vmax, vmax=vmax)
                h=h+1
                ax.set_aspect('equal')
        ...
        fig.colorbar(im, ax=axes.ravel().tolist(),shrink=0.95) #cax=cbar_ax)
        ...
        #     # pylab.colorbar()
        # pylab.colorbar(im, ax=axes.ravel().tolist())
